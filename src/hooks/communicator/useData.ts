...
const useDataState = () =>
{
    ...
    const { sendMessage, lastMessage, readyState } = useWebSocket(GetConfiguration<string>("websocket.external.url"), { share: true });
    ...
    const getResponse = useMemo(() =>
    {
        ...
        if ( lastMessage.origin !== GetConfiguration<string>('websocket.external.url') ) 
        {
